...
* kalmanfilter_ekf_capstone_answer.cpp

When carrying out the Linear Kalman Filter task, use the ```kalmanfilter_lkf.cpp``` as a base and develop all the exercise on top of each other. The file ```kalmanfilter_lkf_answer.cpp``` is a working version that you can use to compare your solution too. The same thing applies to the Extended Kalman Filter and Unscented Kalman Filter exercises with the corresponding files. You can use any of the files as a base for your capstone project.

## Simulation Operation ##
The simulation can be run with different motion and sensor profiles to test the different scenarios and evaluate how the filter performs with different conditions. The different profiles can be activated pressing the number keys 1-9,0, to active the corresponding profile. The different types of Kalman Filters can be selected at run time using the keys: [l, e, u] for Linear, Extended and Unscented alman Filter versions.
...
Starting with the ```kalmanfilter_lkf.cpp``` file. Initialize the filter on first prediction step and then implement the 2D Vehicle process model and Linear Kalman Filter Prediction steps. Test and verify good performance with profile 1.
...
Starting with the ```kalmanfilter_ekf.cpp``` file. Implement the 2D Vehicle process model and Extended Kalman Filter Prediction steps. Test and verify good performance with profiles 1-4.
...
Starting with the ```kalmanfilter_ukf.cpp``` file. Implement the 2D Vehicle Process model and Unscented Kalman Filter Prediction steps. Test and verify good performance with profiles 1-4.
...
### Capstone Project
Starting from any filter base file. Program a filter to provide the best estimation performance for profiles 9,0 (It should also work on any other profiles aswell). You can compare to a solution that I have provided in ```kalmanfilter_ekf_capstone_answer.cpp```.
...
Special thanks to Giovanni Rasera for code improvements and MacOS support.